...
def unrolling(inputs, outputs, input_place_list, output_place_list, extent_dict, name_list, dense):
    ...
    for idx, input_name in enumerate(inputs):
        ...
        glb_reg_write(0x100 * ({input_name_str}_unroll_array[i]) + GLB_LD_DMA_HEADER_0_START_ADDR_R, 0x40000 * ({input_name_str}_unroll_array[i]) + sum);
        }}
        '''
        f.write(dedent(f_str))
    ...
    for idx, input_name in enumerate(inputs):
        ...
        f_str = f'''
        for (int i = 0; i < {input_name_str}_unroll; i++) {{
          glb_reg_write(0x100 * ({input_name_str}_unroll_array[i]) + GLB_LD_DMA_HEADER_0_RANGE_0_R, {input_name_str}_extents[k]-2);
        }}
        '''
        f.write(dedent(f_str))
...
if __name__ == '__main__':
    unrolling(inputs, outputs, input_place_list, output_place_list, extent_dict, name_list, dense)

...
def parse_gold(f, name_list):
    ...
    for gold in name_list:
        gold_list.append(np.load(f"./{gold}/output_gold.npy"))

    for idx, gold in enumerate(gold_list):
        if(not np.any(gold)):
            print("all zeroes")
        f_str = f'''
        uint16_t gold_{idx}_[{gold.shape[0] * gold.shape[1]}] = 
        {{
        '''
    ...
    f_str = f'''
        ...
        for(uint16_t j = 0; j < {gold_list[0].shape[0]}; j++){{
            ...
            printf(\"error! x: %d y:%d gold_ptr:%d check_ptr:%d\\n\", i, j, gold_ptr[{gold_list[0].shape[0]}*i+j], check_ptr[{gold_list[0].shape[0]}*i+j]);
            err++;
            }}
        }}
        }}
